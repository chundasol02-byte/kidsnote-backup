...
이 워크플로는 알림장 외에 **4개의 AI 대시보드**를 LLM(`llama3.1:8b`)으로 자동 생성합니다. 졸업·휴원 시점의 백업이라면 일부 페이지가 어색하게 읽힐 수 있어요 — 특히 모음으로 끝나는 자녀 이름(`유주`, `지우` 등)의 호격을 LLM이 가끔 `유주이야`처럼 잘못 만들어냅니다. **원하지 않는 페이지만 골라서 끌 수 있습니다.**
...
**한 번만 끄기 (이번 실행만)**:
1. fork repo → `Actions` 탭 → `Kidsnote → Notion mirror` 클릭
2. `Run workflow ▾` 클릭
3. 원하는 옵션의 드롭다운을 `false` → `true`로 변경
4. 녹색 `Run workflow` 클릭

**매번 끄기 (영구)**: `.github/workflows/kidsnote-to-notion.yml` 파일을 편집해서 원하는 옵션의 `default: "false"`를 `default: "true"`로 바꾼 후 commit. 이후로는 `Run workflow` 화면에서 매번 `true`가 미리 선택되어 있습니다.

### 💡 상황별 권장 조합

| 상황 | 권장 |
|---|---|
| 재원 중인 자녀, 모든 페이지 다 원함 | 4개 모두 기본값 `false` (=AI 페이지 모두 생성) |
| 졸업·휴원 후 백업 — 편지가 어색함 | `disable_growth_story=true` + `disable_teacher_thanks=true` (편지 성격만 차단, 통계 페이지는 유지) |
| AI 페이지 전부 싫음, 통계만 원함 | 4개 모두 `true` (📊 통계 / 📅 추억 / 🥗 영양 세 페이지는 LLM이 아닌 결정론적 통계라 그대로 생성됨) |
| 호격 오류가 문제 (이름이 모음으로 끝남) | 최소 `disable_growth_story=true` + `disable_teacher_thanks=true` 권장 |

> 💡 **AI 페이지 전부 끄면 워크플로 실행 시간이 짧아집니다**. Ollama 모델 다운로드(~4.7GB)는 여전히 일어나지만 LLM 호출이 없어 1년치 백업도 보통 시간이 절반 이하로 줄어요.
