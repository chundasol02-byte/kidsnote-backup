...
기본 셋업은 **자녀 1명 + 4개의 AI 대시보드 모두 켜짐** 상태로 동작합니다. 아래 두 가지가 해당되는 경우만 읽어주세요.

## A. 자녀가 둘 이상이고, 첫째가 아닌 다른 아이를 백업하고 싶을 때

키즈노트 계정에 자녀가 여럿이면 워크플로는 **API가 첫 번째로 돌려준 아이**를 기본으로 백업합니다. 키즈노트 웹에서 "자녀 전환"하고 쿠키를 다시 추출해도 — 쿠키 자체가 자녀별로 다르지 않기 때문에 — 결과가 바뀌지 않습니다. 다른 아이를 백업하려면 **워크플로 input으로 명시적으로 지정**해야 합니다.

> 💡 **자녀별로 노션 DB는 별도로 만드는 것을 권장합니다.** 같은 DB에 여러 아이 알림장이 섞이면 통계·대시보드가 한 아이 기준으로 만들어지므로 의미가 옅어집니다.

### 1. 자녀별 새 노션 DB 만들기
[2단계](#2단계-노션에-백업용-데이터베이스-만들기) ~ [5단계](#5단계-노션-db-id-복사하기)를 둘째 자녀용으로 다시 진행해서 **새로운 DB ID**를 얻습니다.

### 2. fork repo에 둘째 자녀용 workflow를 별도로 만들기 OR 같은 워크플로를 두 번 실행

**방법 1 (권장) — 한 fork에서 자녀 전환하며 사용**:
- fork repo → `Actions` 탭 → `Kidsnote → Notion mirror` → `Run workflow ▾`
- `child_name`에 백업하려는 자녀 이름의 일부를 입력 (예: `유주`)
- 같은 화면에서 `NOTION_DATABASE_ID` secret을 둘째 DB ID로 임시 교체 후 실행
- 단점: 자녀를 바꿀 때마다 secret을 손으로 바꿔야 함

**방법 2 — fork를 두 개 만들어 자녀별로 분리** (가장 안정적):
- 첫째용 fork와 둘째용 fork를 각각 만들어 각 fork에 해당 자녀의 secret을 설정
- 두 fork가 독립적으로 동작 (쿠키는 같아도 됨)

### 3. workflow input — `child_name`

`Run workflow` 화면에 새로 생긴 `child_name` 칸에 자녀 이름의 **일부**를 입력합니다 (대소문자 무관, 한국어 받침 정확히 안 맞아도 됨). 비워두면 첫 번째 자녀.

| `child_name` 입력 | 동작 |
|---|---|
| (비워둠) | 첫 번째 자녀 (기본 동작) |
| `유주` | 이름에 "유주"가 들어간 자녀 1명 백업 |
| (모호하게 매칭됨) | "유주"가 들어간 자녀가 2명 이상이면 에러 — 정확한 ID로 지정하라는 메시지 출력 |

### 4. 실행 로그에서 자녀 ID 확인하기

워크플로를 한 번 돌리면 시작 직후 로그에 모든 자녀가 나옵니다:
```
Account has 2 child(ren): #1 id=5000421 name=우유준 #2 id=5000422 name=우유주
Selected child: id=5000421 name=우유준
```
이 ID·이름을 보고 `child_name`을 정확히 채울 수 있습니다.

---

## B. AI가 만드는 편지·스토리를 끄고 싶을 때 (졸업생 백업 등)

이 워크플로는 알림장 외에 **4개의 AI 대시보드**를 자동 생성합니다. 졸업·휴원 시점의 백업이라면 일부 페이지가 어색하게 읽힐 수 있어요 — 특히 모음으로 끝나는 자녀 이름(`유주`, `지우` 등)의 호격을 LLM이 가끔 `유주이야`처럼 잘못 만들어냅니다. 원하지 않는 페이지만 골라서 끌 수 있습니다.

| input | 끄면 안 만들어지는 페이지 | 영향받는 호격 오류 |
|---|---|---|
| `disable_growth_story` | 📖 매월 성장 스토리 | 자녀 이름이 본문에 자주 나오므로 영향 큼 |
| `disable_milestones` | 🌟 마일스톤 | 본문에 자녀 이름 등장 적음 |
| `disable_interests` | 🌱 분기 관심사 | 통계성이라 호격 오류 없음 |
| `disable_teacher_thanks` | 💌 선생님께 감사 카드 | 자녀 이름 본문 중 1-2회 등장 |

**졸업 백업 권장 조합**: `disable_growth_story=true` + `disable_teacher_thanks=true` (편지 성격이 강한 두 페이지만 끄고 마일스톤·관심사는 유지)

**전부 끄고 싶다면**: 네 개 모두 `true`로 설정. 📊 통계 / 📅 추억 / 🥗 영양 세 페이지는 LLM이 아닌 결정론적 통계라 그대로 만들어집니다.

> 💡 이 옵션들은 **실행할 때마다 따로 지정**합니다. 매번 같은 조합을 쓰고 싶다면 `.github/workflows/kidsnote-to-notion.yml`의 `default: "false"`를 `default: "true"`로 바꿔서 commit하세요.
...
내부적으로 사용하는 LLM(`llama3.1:8b`)이 모음으로 끝나는 자녀 이름(`유주`, `지우` 등)의 호격 조사를 가끔 잘못 만들어냅니다. 받침 있는 이름(`하린`, `시연` 등)에는 일어나지 않습니다.

해결 방법은 두 가지:
1. **해당 페이지만 끄기** — `Run workflow`의 `disable_growth_story` / `disable_teacher_thanks`를 `true`로 (가장 영향 큰 두 페이지)
2. **AI 대시보드 전부 끄기** — 네 개 `disable_*` 옵션 모두 `true`로 (졸업·휴원 백업 권장)
